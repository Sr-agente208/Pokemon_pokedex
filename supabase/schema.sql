...
create table if not exists public.profiles (
  ...
  nome text not null,
  email text not null,
  cargo text not null default 'Usuário' check (cargo in ('Usuário','Assistente','ADM','Analista de Sistema','Sub Dono','Dono')),
  ativo boolean not null default true,
  created_at timestamptz not null default now()
);
create table if not exists public.verification_codes (
  code text primary key,
  cargo text not null check (cargo in ('Assistente','ADM','Analista de Sistema','Sub Dono','Dono')),
...
declare generated text; begin
  if (select p.cargo from profiles p where p.id=auth.uid() and p.ativo=true) not in ('ADM','Dono') then raise exception 'Apenas ADM ou Dono pode gerar códigos'; end if;
  if requested_cargo not in ('ADM','Assistente','Analista de Sistema','Sub Dono','Dono') then raise exception 'Cargo inválido'; end if;
