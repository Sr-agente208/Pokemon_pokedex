...
function normalizeCargo(cargo) { return ["Dono", "Sub Dono", "Analista de Sistema", "ADM", "Assistente", "Usuário"].indexOf(cargo) >= 0 ? cargo : "Usuário"; }
...
const db = {
  ...
  async login(email, senha) { if (online) return (await pool.query("SELECT id,nome,email,cargo FROM usuarios WHERE email=$1 AND senha=$2", [String(email).toLowerCase(),senha])).rows[0]; const user=read("usuarios").find(u => u.email === String(email).toLowerCase() && u.senha === senha); return user ? {id:user.id,nome:user.nome,email:user.email,cargo:user.cargo} : null; },
