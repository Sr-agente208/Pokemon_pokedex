...
(function () {
  ...
  window.api = function (method, url, body, ok, fail) {
    ...
    if (window.supabaseClient) {
      ...
      if(method==='POST' && url==='/recados') return sb.functions.invoke('moderate-recado',{body:{mensagem:body.mensagem||body.texto}}).then(function(r){if(r.error){if(fail)fail({erro:r.error.message||'Não foi possível analisar o recado.'});return;}if(r.data&&r.data.status!=='APPROVED'){if(fail)fail({erro:'Treinador Nébula: '+(r.data.reason||'recado precisa de revisão.')});return;}if(ok)ok(r.data);});
      if(method==='DELETE' && url.indexOf('/recados/')===0) return sb.from('recados').delete().eq('id',url.split('/').pop()).select('id').then(function(r){if(r.error||!r.data||!r.data.length){if(fail)fail({erro:r.error?r.error.message:'Não foi possível excluir este recado.'});return;}if(ok)ok({sucesso:true});});
