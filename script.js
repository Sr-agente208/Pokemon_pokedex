...
(function () {
  ...
  window.api = function (method, url, body, ok, fail) {
    ...
    if (window.supabaseClient) {
      ...
      if(method==='DELETE' && url.indexOf('/recados/')===0) return sb.rpc('delete_recado_as_staff',{target_id:Number(url.split('/').pop())}).then(function(r){if(r.error||!r.data){if(fail)fail({erro:r.error?r.error.message:'Não foi possível excluir este recado.'});return;}if(ok)ok({sucesso:true});});
